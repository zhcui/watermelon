...
def create(dp, D=None, fn=np.zeros, full_tensor = True):
    """
    ...
    dim_rs = calc_dim(_dp, D, full_tensor = full_tensor)
...
def empty(dp, D = None, full_tensor = True):
    return create(dp, D, fn=np.empty, full_tensor = full_tensor)

def zeros(dp, D = None, full_tensor = True):
    return create(dp, D, fn=np.zeros, full_tensor = full_tensor)

def rand(dp, D = None, seed = None, full_tensor = True):
    if seed is not None:
        np.random.seed(seed)
    return create(dp, D, fn=np.random.random, full_tensor = full_tensor)


def calc_dim(dps, D = None, full_tensor = True):

...
def create(dp, D = None, fn = np.zeros, **kwargs):
    """
    ...
    block_shape = (1, _dp[0], 1)
    mpx[0] = fn((1, _dp[0], dim_rs[0]), block_shape = block_shape, **kwargs)
    #mpx[0]  = fn((1, _dp[0], dim_rs[0]))
    for i in range(1, L-1):
        block_shape = (1, _dp[i], 1)
        mpx[i] = fn((dim_rs[i-1], _dp[i], dim_rs[i]), block_shape = block_shape, **kwargs)
    block_shape = (1, _dp[-1], 1)
    mpx[-1] = fn((dim_rs[-1], _dp[-1], 1), block_shape = block_shape, **kwargs)
...
def rand(dp, D = None, density = 1.0, seed = None):
    if seed is not None:
        np.random.seed(seed)
    return create(dp, D, fn = sparse.brandom, density = density)
